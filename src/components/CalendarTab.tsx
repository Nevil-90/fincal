...
function CalendarTab({}: CalendarTabProps) {
  ...
  const getDayIntensity = (date: Date): string => {
    ...
    if (total === 0) return ''
    if (total <= 500) return 'bg-rose-50/40'
    if (total <= 2000) return 'bg-rose-50/70'
    if (total <= 5000) return 'bg-rose-100/60'
    return 'bg-rose-100'
  }
    ...
    return (
      ...
      >
        ...
        <div className="text-right shrink-0">
          ...
          {t.paymentMethod && (
            <p className="text-[10px] text-slate-400 mt-0.5">{t.paymentMethod}</p>
          )}
        </div>
      </button>
    )
    ...
    <div className={`${CARD} overflow-hidden`}>
      ...
      <div className="grid grid-cols-7 border-b border-slate-100">
        {DAY_LABELS.map(label => (
          <div key={label} className="px-2 py-3 text-center text-[11px] font-bold text-slate-400 uppercase tracking-widest">
            {label}
          </div>
        ))}
      </div>
        ...
        {calendarDays.map(day => {
          ...
          return (
            <div
              ...
              className={`
                min-h-[60px] sm:min-h-[130px] border-b border-r border-slate-100 p-1 sm:p-2 cursor-pointer
              ...
              <div className="flex items-start justify-between mb-0.5 sm:mb-1.5">
                <span className={`
                  ...
                  ${today
                    ...
                    : 'h-5 w-5 sm:h-7 sm:w-7 rounded-full text-slate-700'
                  }
                `}>
                  {format(day, 'd')}
                </span>
                ...
                {dayTx.length > 0 && (
                  <div className="hidden sm:flex flex-col items-end gap-0.5">
                    {dayIncome > 0 && (
                      <span className="text-[10px] font-bold text-emerald-600">+{formatCurrency(dayIncome)}</span>
                    )}
                    {dayExpense > 0 && (
                      <span className="text-[10px] font-bold text-rose-600">-{formatCurrency(dayExpense)}</span>
                    )}
                  </div>
                )}
              </div>
        ...
        <div className="min-w-[640px]">
          <div className="grid grid-cols-7 border-b border-slate-100">
            {weekDays.map(day => {
              const today = isToday(day)
              return (
                <div key={day.toISOString()} className={`px-3 py-3 text-center border-r border-slate-100 last:border-r-0 ${today ? 'bg-blue-50' : ''}`}>
                  <p className="text-[10px] font-bold text-slate-400 uppercase">{format(day, 'EEE')}</p>
                  <p className={`text-lg font-black mt-0.5 ${today ? 'text-blue-600' : 'text-slate-900'}`}>
                    {format(day, 'd')}
                  </p>
                </div>
              )
          ...
          <div className="grid grid-cols-7 min-h-[400px]">
            {weekDays.map(day => {
              ...
              return (
                <div key={key} className="border-r border-slate-100 last:border-r-0 p-2 space-y-1.5">
                  {dayTx.length === 0 ? (
                    <p className="text-[10px] text-slate-300 text-center mt-4">No activity</p>
                  ) : (
            ...
            <div key={dateKey} className={CARD}>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
                <div className="flex items-center gap-3">
                  <div className={`flex flex-col items-center justify-center h-11 w-11 rounded-xl ${isToday(date) ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700'}`}>
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">{format(date, 'EEEE, MMMM d')}</p>
                    <p className="text-[11px] text-slate-500">{txs.length} transaction{txs.length !== 1 ? 's' : ''}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {dayIncome > 0 && (
  ...
  return (
    <div className="space-y-5 font-sans pb-24 md:pb-0">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-2xl font-black tracking-tight text-slate-900">Financial Calendar</h2>
          <p className="text-sm text-slate-500 mt-0.5">Track spending patterns and financial activity over time.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1">
            ...
            <span className="px-3 text-sm font-bold text-slate-900 min-w-[150px] text-center">
              {headerLabel}
            </span>
          ...
          <button
            onClick={goToday}
            className="px-3.5 py-2 text-xs font-bold rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all"
          >
            Today
          </button>
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-all ${showFilters ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-200 bg-white text-slate-500 hover:text-slate-700'}`}
          >
      ...
      {view !== 'agenda' && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className={`${CARD} p-4`}>
            ...
            <p className="text-lg font-black text-emerald-600">{formatCurrency(monthStats.income)}</p>
          </div>
          ...
          <div className={`${CARD} p-4`}>
            ...
            <p className="text-lg font-black text-rose-600">{formatCurrency(monthStats.expense)}</p>
          </div>

          <div className={`${CARD} p-4`}>
            <div className="flex items-center gap-2 mb-1">
              <Banknote className="h-3.5 w-3.5 text-blue-500" />
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Net Flow</span>
            </div>
            <p className={`text-lg font-black ${monthStats.net >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
          ...
          <div className={`${CARD} p-4`}>
            ...
            <p className="text-lg font-black text-slate-900">{monthStats.txCount}</p>
          </div>
        </div>
      )}
      ...
      {fetchLoading && (
        <div className={`${CARD} p-12 text-center animate-pulse`}>
          <p className="text-sm text-slate-400 font-semibold">Loading financial calendar...</p>
        </div>
      )}
      ...
      {!fetchLoading && (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
          ...
          <div className="space-y-4">
              ...
              <div className={CARD}>
                <div className="px-4 py-3 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-slate-900">Upcoming (7 days)</h3>
                  </div>
                </div>
                <div className="p-3 space-y-2">
                  {upcoming.map(t => {
            ...
            {/* Legend */}
            <div className={`${CARD} p-4`}>
              <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">Legend</h3>
              <div className="space-y-2">
                ...
                <div className="border-t border-slate-100 pt-2 mt-2">
                  <p className="text-[10px] text-slate-400">Cells with a pink tint indicate higher spending days.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center" onClick={() => setSelectedDay(null)}>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className={`flex flex-col items-center justify-center h-12 w-12 rounded-xl ${isToday(selectedDay) ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700'}`}>
                ...
                <div>
                  <p className="text-base font-bold text-slate-900">{format(selectedDay, 'EEEE, MMMM d')}</p>
                  <p className="text-xs text-slate-500">{format(selectedDay, 'yyyy')}</p>
                </div>
              </div>
              <button onClick={() => setSelectedDay(null)} className="p-2 rounded-full hover:bg-slate-100 transition-colors">
                ...
                <>
                  <div className={`px-6 py-5 ${theme.bg} border-b ${theme.border}`}>
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`flex items-center justify-center h-10 w-10 rounded-xl bg-white/60 ${theme.text}`}>
                          <Icon className="h-5 w-5" />
                        </div>
                        <div>
                          <p className={`text-sm font-bold ${theme.text}`}>{theme.label}</p>
                          <p className="text-[11px] text-slate-500">{format(new Date(t.date), 'EEEE, MMM d, yyyy · hh:mm a')}</p>
                        </div>
                      </div>
                      <button onClick={() => setSelectedTransaction(null)} className="p-2 rounded-full hover:bg-white/50 transition-colors">
                        <X className="h-4 w-4 text-slate-500" />
                      </button>
                    </div>
                  ...
                  <div className="px-6 py-5 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                        ...
                        <p className="text-sm font-bold text-slate-900 mt-1">{t.paymentMethod || '—'}</p>
                      </div>
                      {t.description && (
                      ...
                      {t.source && (
                        <div className="col-span-2 p-3 rounded-xl bg-slate-50 border border-slate-100">
                          ...
                          <p className="text-sm font-bold text-slate-900 mt-1">{t.source}</p>
                        </div>
                      )}
                    </div>
                  </div>
