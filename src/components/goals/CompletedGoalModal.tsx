  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4 lg:sticky lg:top-6 lg:inset-auto lg:z-auto lg:p-0 lg:block lg:w-[420px] xl:w-[450px] lg:shrink-0 lg:flex-none">
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-t-[32px] sm:rounded-3xl w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] lg:max-h-none lg:max-w-none relative z-10 animate-slide-up lg:animate-none shadow-2xl lg:shadow-[0_8px_30px_rgb(0,0,0,0.04)] lg:border lg:border-gray-100 dark:lg:border-neutral-800 flex flex-col overflow-hidden">
        ...
        <div className="overflow-y-auto min-h-0 overscroll-contain bg-gray-50 dark:bg-neutral-950 p-6 pb-safe">
          <h5 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <History className="h-5 w-5 text-gray-500 dark:text-neutral-400" /> Contribution Journey
          </h5>
          <div className="bg-white dark:bg-neutral-900 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-neutral-800">
            {loadingContributions === selectedCompletedGoal ? (
              <div className="animate-pulse space-y-4">
                ...
                <div className="h-4 bg-gray-100 dark:bg-neutral-800/50 rounded w-1/4"></div>
              </div>
            ) : (
              <div className="space-y-4">
                {(contributions[selectedCompletedGoal] || g.contributions || []).map((c) => (
                  <div key={c.id} className="flex justify-between items-center p-3 -mx-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-neutral-800/50">
                    <div className="flex items-center gap-4">
                      <div className="h-10 w-10 rounded-full bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 flex items-center justify-center font-bold text-sm">
                        +
                      </div>
                      <div>
                ...
                {(!contributions[selectedCompletedGoal] && !g.contributions?.length) && (
                  <div className="text-center py-8">
                    <p className="text-gray-500 dark:text-neutral-400 font-medium">No contribution data found</p>
                  </div>
                )}
              </div>
            )}
