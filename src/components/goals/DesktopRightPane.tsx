...
export function DesktopRightPane({
  ...
  if (goalFilter === 'active' && selectedGoal) {
    ...
    return (
      <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden flex flex-col animate-slide-in h-full">
        {/* Header */}
        <div className="p-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
          <div>
            <div className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 rounded-full text-[10px] font-bold uppercase tracking-wider text-purple-600 mb-1">
              <Target className="h-3 w-3" /> Active Goal
            </div>
            <h3 className="text-base font-bold text-slate-900 leading-tight truncate max-w-[280px]">{g.name}</h3>
            {g.deadline && (
          ...
          <div className="flex items-center gap-1.5 ml-4">
            <button
              ...
              className="p-1.5 bg-slate-100 text-slate-400 hover:bg-rose-100 hover:text-rose-600 rounded-xl transition-colors cursor-pointer"
              title="Delete Goal"
            >
            ...
            <button
              ...
              className="p-1.5 bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-600 rounded-xl transition-colors cursor-pointer"
              title="Close Panel"
            >
              <X className="h-4 w-4" />
            </button>
        ...
        {/* Stats Bar */}
        <div className="p-3 border-b border-slate-100 bg-white grid grid-cols-2 gap-4 shrink-0">
          <div>
            <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider mb-0.5">Current Saved</p>
            <p className="text-lg font-black text-slate-900">{formatCurrency(g.currentAmount)}</p>
            <p className="text-[11px] text-slate-400 mt-0.5">of {formatCurrency(g.targetAmount)} target</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider mb-0.5">Remaining</p>
            <p className="text-lg font-black text-slate-900">{formatCurrency(remaining)}</p>
            {pace > 0 ? (
              <p className="text-[11px] text-purple-600 font-bold mt-0.5">Requires {formatCurrency(pace)}/mo</p>
            ) : (
              <p className="text-[11px] text-slate-400 mt-0.5">No deadline set</p>
            )}
          </div>
        </div>
        ...
        <div className="p-3 flex-1 min-h-0 flex flex-col gap-2">
          ...
          <div className="flex p-1 bg-slate-100 rounded-xl shrink-0">
            <button
              ...
              type="button"
              className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${detailTab === 'quick' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Add Contribution
            </button>
            <button
              ...
              type="button"
              className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${detailTab === 'history' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Activity History
            </button>
          </div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:bg-white transition-all text-slate-900 font-bold outline-none text-xs h-[40px]"
                    required
                  />
                </div>

                <div className="relative w-full">
                  <div className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-semibold flex items-center justify-between pointer-events-none transition-colors text-xs h-[40px]">
                    <span className={quickContribution.date ? 'text-slate-900' : 'text-slate-400'}>
                ...
                <div className="grid grid-cols-4 gap-1.5 h-[40px]">
                  {[500, 1000, 5000, 10000].map((amt) => (
                    <button
                      ...
                      className={`h-full text-[11px] font-bold border rounded-xl transition-all ${quickContribution.amount === amt.toString() ? 'border-purple-600 bg-purple-50 text-purple-700 font-extrabold shadow-sm' : 'border-slate-200 hover:border-slate-300 bg-white text-slate-600'}`}
                    >
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:bg-white transition-colors text-slate-900 font-semibold outline-none text-xs cursor-pointer h-[40px]"
                >
              ...
              <div className="flex gap-4 items-center">
                <div className="flex-1">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:bg-white transition-colors text-slate-900 font-medium outline-none text-xs h-[40px]"
                  />
                </div>
                <button
                  type="submit"
          ...
          {detailTab === 'history' && (
            <div className="space-y-3 flex-1 min-h-0 flex flex-col">
              <h5 className="font-bold text-slate-800 text-[11px] flex items-center gap-1.5 border-b border-slate-100 pb-1.5 mb-2">
                ...
                <div className="space-y-2 overflow-y-auto pr-1 custom-scrollbar">
                  {contributions[selectedGoal]?.length > 0 ? (
                    contributions[selectedGoal].map((c) => (
                      <div key={c.id} className="flex justify-between items-center group p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <p className="font-extrabold text-slate-900 text-xs">+{formatCurrency(c.amount)}</p>
                            <p className="text-[10px] font-semibold text-slate-400 truncate max-w-[180px] mt-0.5">
                    ...
                    <div className="text-center py-8">
                      <div className="h-8 w-8 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-2">
                        <History className="h-4 w-4 text-slate-400" />
                      </div>
                      <p className="text-slate-500 font-bold text-xs">No activity logged</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Make your first deposit to track historical growth.</p>
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden flex flex-col animate-slide-in h-full">
        ...
        <div className="p-4 bg-gradient-to-br from-emerald-500 to-indigo-600 text-white flex justify-between items-center relative shrink-0">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-bold uppercase tracking-wider mb-1">
          ...
          <div className="flex items-center gap-1.5 ml-4 relative z-10">
            <button
              onClick={() => handleDeleteGoal(g)}
              className="p-1.5 bg-white/10 hover:bg-rose-500/80 text-white rounded-lg transition-colors cursor-pointer"
              title="Delete Goal"
            >
              <Trash2 className="h-4 w-4" />
            </button>
        ...
        <div className="p-4 space-y-3 bg-slate-50/50 flex-1 min-h-0 flex flex-col overflow-hidden">
          <h5 className="font-bold text-slate-800 text-[11px] flex items-center gap-1.5 border-b border-slate-100 pb-1.5 shrink-0">
            <History className="h-3.5 w-3.5 text-emerald-500" /> Historical Deposits
          </h5>
          <div className="bg-white rounded-2xl p-4 border border-slate-100 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {loadingContributions === selectedCompletedGoal ? (
              ...
              <div className="space-y-2">
                {(contributions[selectedCompletedGoal] || g.contributions || []).map((c) => (
                  <div key={c.id} className="flex justify-between items-center p-2 rounded-xl hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-2.5">
                      <div className="h-8 w-8 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-black text-[10px]">
                        ✓
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 text-xs">+{formatCurrency(c.amount)}</p>
                ...
                {(!contributions[selectedCompletedGoal] && !g.contributions?.length) && (
                  <p className="text-center py-6 text-slate-400 text-[11px] font-semibold">No deposition data stored.</p>
                )}
              </div>
            )}
          </div>
