...
export default function AnalyticsTab({ goals }: AnalyticsTabProps) {
  ...
  const getHeatColor = (cell: { income: number; expense: number; total: number }): string => {
    if (cell.total === 0) return 'bg-slate-100'
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-12 font-sans bg-slate-50 min-h-screen -mx-4 -mt-4 px-3 py-5 sm:p-8 rounded-tl-3xl">

      {/* Dashboard Header */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-2">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-slate-900">Financial Intelligence</h1>
          <p className="text-sm font-semibold text-slate-500 mt-1">Professional analytics and data deep-dives.</p>
        </div>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto overflow-hidden">
          <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm overflow-x-auto w-full sm:w-auto" style={{ scrollbarWidth: 'none' }}>
            {(['this_month', 'last_3_months', 'last_6_months', 'last_12_months', 'ytd', 'all_time'] as DateFilter[]).map(f => {
              ...
              return (
                <button key={f} onClick={() => setDateFilter(f)}
                  className={`flex-1 text-center px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap ${dateFilter === f ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
                >
                  {labelMap[f]}
                </button>
              )
      ...
      <div className="overflow-x-auto -mx-1 px-1" style={{ scrollbarWidth: 'none' }}>
        <div className="flex items-center gap-1 sm:gap-2 pb-2 border-b border-slate-200 min-w-max">
          {(['overview', 'cashflow', 'compare', 'categories', 'behaviors', 'forecasting'] as SubTab[]).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-3 sm:px-4 py-2 sm:py-2.5 rounded-t-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap -mb-[1px] border-b-2 ${activeTab === tab ? 'text-blue-600 border-blue-600 bg-blue-50/50' : 'text-slate-500 border-transparent hover:text-slate-800 hover:bg-slate-100/50'}`}
            >
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 pt-2 [&>*:last-child:nth-child(odd)]:col-span-2 lg:[&>*:last-child:nth-child(odd)]:col-span-1">
        <div className={cardClasses}>
          <div className="flex items-center gap-2 mb-1"><Activity className="h-4 w-4 text-slate-400" /><h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Net Cash Flow</h3></div>
          <span className={`text-2xl font-black tracking-tight ${kpis.netFlow >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{formatCurrency(kpis.netFlow)}</span>
        </div>
        <div className={cardClasses}>
          <div className="flex items-center gap-2 mb-1"><TrendingUp className="h-4 w-4 text-slate-400" /><h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Savings Rate</h3></div>
          <div className="flex items-baseline gap-1"><span className="text-2xl font-black tracking-tight text-slate-900">{kpis.savingsRate.toFixed(1)}%</span><span className="text-xs font-semibold text-slate-500">of income</span></div>
        </div>
        <div className={cardClasses}>
          <div className="flex items-center gap-2 mb-1"><Zap className="h-4 w-4 text-slate-400" /><h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Avg Daily Burn</h3></div>
          <div className="flex items-baseline gap-1"><span className="text-2xl font-black tracking-tight text-slate-900">{formatCurrency(kpis.burnRate)}</span><span className="text-xs font-semibold text-slate-500">/ day</span></div>
        </div>
        <div className={cardClasses}>
          <div className="flex items-center gap-2 mb-1"><Target className="h-4 w-4 text-slate-400" /><h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Active Goals</h3></div>
          <span className="text-2xl font-black tracking-tight text-slate-900">{goals.filter(g => !g.isCompleted).length}</span>
        </div>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className={cardClasses + ' p-6 sm:p-7 relative overflow-hidden shadow-md'}>
              <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-900">Financial Activity Timeline</h3>
                    <p className="text-xs text-slate-500 mt-0.5">365-day cashflow velocity heatmap</p>
                  </div>
                </div>
                ...
                <div style={{ minWidth: `${(heatmapGrid.totalWeeks || 53) * 22 + 45}px` }}>
                  <div className="flex items-end ml-[42px] mb-2.5 h-4 relative">
                    {heatmapGrid.months?.map((m: any, i: number) => (
                      <div key={`m-${i}`} className="absolute text-xs font-bold text-slate-400" style={{ left: `${m.col * 22}px` }}>{m.label}</div>
                    ))}
                  </div>
                  <div className="flex gap-0">
                    <div className="flex flex-col justify-between pr-2.5 shrink-0" style={{ height: `${7 * 22 - 4}px`, width: '38px' }}>
                      {['Sun', '', 'Tue', '', 'Thu', '', 'Sat'].map((lbl, i) => (
                        <span key={i} className="text-xs font-bold text-slate-400 leading-none" style={{ height: '18px', display: 'flex', alignItems: 'center' }}>{lbl}</span>
                      ))}
                    </div>
                      ...
                      {heatmapGrid.rows?.map((row: any, rowIdx: number) => (
                        <div key={rowIdx} className="flex gap-[4px]">
                          {row.map((cell: any, colIdx: number) => {
                            if (!cell) return <div key={`e-${rowIdx}-${colIdx}`} className="h-[18px] w-[18px] rounded-[5px] bg-slate-50/50" />
              ...
              {/* Heatmap Legend */}
              <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-slate-100 pt-4 text-xs font-bold text-slate-400">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-[3px]">
                    <div className="h-3.5 w-3.5 rounded-[4px] bg-slate-100" />
                    <div className="h-3.5 w-3.5 rounded-[4px] bg-blue-100" />
              ...
              {heatmapTooltip && (
                <div className="fixed z-50 pointer-events-none px-3 py-2 rounded-lg bg-slate-900 text-white text-xs font-semibold shadow-xl" style={{ left: `${heatmapTooltip.x}px`, top: `${heatmapTooltip.y}px`, transform: 'translate(-50%, -100%)' }}>
                  ...
                  {heatmapTooltip.income === 0 && heatmapTooltip.expense === 0 && <p className="text-slate-400 mt-0.5">No activity</p>}
                </div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-2 xl:[&>*:last-child:nth-child(odd)]:col-span-2">
            {/* Monthly Trend */}
            <div className={cardClasses}>
              <div className="mb-6 flex items-center gap-2"><BarChart2 className="h-5 w-5 text-blue-600" /><div><h3 className={headerClasses}>Income vs Expense Trend</h3><p className={subHeaderClasses}>Last 6 months comparison</p></div></div>
            ...
            {/* Payment Methods */}
            <div className={cardClasses}>
              <div className="mb-6 flex items-center gap-2"><PieChartIcon className="h-5 w-5 text-blue-600" /><div><h3 className={headerClasses}>Payment Methods</h3><p className={subHeaderClasses}>How you pay for things.</p></div></div>
              {(() => {
                const data = paymentMethodData
                if (data.length === 0) return <div className="flex h-48 items-center justify-center text-slate-400 text-sm font-semibold">No expense data.</div>
                return (
                  <div className="h-72 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
        ...
        <div className="grid grid-cols-1 gap-6 animate-fadeIn">
          <div className={cardClasses}>
            ...
            <div className="h-[250px] sm:h-[400px] w-full">
              ...
              ) : <div className="flex h-full items-center justify-center text-slate-400 text-sm font-semibold">Insufficient data to map flow.</div>}
            </div>
          </div>

          <div className={cardClasses}>
            <div className="mb-6 flex items-center gap-2"><TrendingUp className="h-5 w-5 text-blue-600" /><div><h3 className={headerClasses}>Cumulative Trajectory</h3><p className={subHeaderClasses}>Historical timeline of income, expenses, and cumulative balance.</p></div></div>
            <div className="h-64 sm:h-80 w-full">
              ...
              ) : <div className="flex h-full items-center justify-center text-slate-400 text-sm font-semibold">No timeline data available.</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'categories' && (
        <div className="grid grid-cols-1 gap-6 animate-fadeIn">
          <div className={cardClasses}>
            ...
            {(() => {
              ...
              if (sorted.length === 0) return (
                <div className="flex h-40 items-center justify-center text-slate-400 text-sm font-semibold">No expense data available.</div>
              )
              return (
                      ...
                      <div key={cat} className="group">
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="text-xs sm:text-sm font-semibold text-slate-800 truncate">{cat}</span>
                          </div>
                          <div className="flex items-center gap-3 shrink-0 ml-3">
                            <span className="text-[10px] font-bold text-slate-400">{pct.toFixed(1)}%</span>
                            <span className="text-xs sm:text-sm font-black text-slate-900 tabular-nums">{formatCurrency(amount)}</span>
                          </div>
                        </div>
          ...
          <div className={cardClasses}>
            <div className="mb-6 flex items-center gap-2"><TableIcon className="h-5 w-5 text-slate-600" /><div><h3 className={headerClasses}>Cross-Tab Matrix (Pivot Data)</h3><p className={subHeaderClasses}>Exact numerical spending per category over the last 6 months.</p></div></div>
            <div className="overflow-x-auto overscroll-contain border border-slate-200 rounded-xl scrollbar-thin">
              <table className="w-full min-w-[640px] sm:min-w-full text-left text-xs sm:text-sm text-slate-600">
                <thead className="bg-slate-50 text-[10px] sm:text-xs uppercase text-slate-700 border-b border-slate-200">
                  <tr>
                    <th className="sticky left-0 z-20 bg-slate-50 px-2.5 py-2.5 sm:px-4 sm:py-3 font-bold border-r border-slate-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.08)]">Category</th>
                    ...
                    <th className="px-2.5 py-2.5 sm:px-4 sm:py-3 font-bold text-right bg-blue-50 whitespace-nowrap">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {pivotTableData.rows.length > 0 ? pivotTableData.rows.map((row: any, idx: number) => (
                    <tr key={row.category} className={`border-b border-slate-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                      <td className={`sticky left-0 z-10 px-2.5 py-2 sm:px-4 sm:py-3 font-semibold border-r border-slate-200 text-xs sm:text-sm shadow-[2px_0_5px_-2px_rgba(0,0,0,0.08)] ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>{row.category}</td>
                      ...
                      <td className="px-2.5 py-2 sm:px-4 sm:py-3 font-bold text-right tabular-nums bg-blue-50 text-blue-700 text-xs sm:text-sm whitespace-nowrap">{formatCurrency(row.total)}</td>
                    </tr>
                  )) : <tr><td colSpan={pivotTableData.columns.length + 2} className="px-4 py-8 text-center text-slate-400">No data available for pivot view.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      {activeTab === 'behaviors' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-fadeIn xl:[&>*:last-child:nth-child(odd)]:col-span-2">
          <div className={`xl:col-span-2 ${cardClasses}`}>
            <div className="mb-6 flex items-center gap-2"><ShieldAlert className="h-5 w-5 text-red-600" /><div><h3 className={headerClasses}>Anomaly Detection Engine</h3><p className={subHeaderClasses}>Identifies unusual spending spikes outside your standard deviation (Red points).</p></div></div>
            <div className="h-[250px] sm:h-[400px] w-full">
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
                              <p className="font-bold text-slate-900">{data.description}</p>
                              <p className="text-xs font-semibold text-slate-500 uppercase mt-1">{data.category}</p>
                              <p className={`text-lg font-black mt-2 ${data.isAnomaly ? 'text-red-600' : 'text-blue-600'}`}>{formatCurrency(data.amount)}</p>
                              <p className="text-xs text-slate-400 mt-1">{data.date}</p>
                              {data.isAnomaly && <div className="mt-2 flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-50 p-1.5 rounded-md uppercase"><AlertTriangle className="h-3 w-3" /> Statistical Anomaly</div>}
                            </div>
                          )
                        }
                        return null
              ...
              ) : <div className="flex h-full items-center justify-center text-slate-400 text-sm font-semibold">Insufficient data for anomaly detection.</div>}
            </div>
          </div>

          <div className={cardClasses}>
            <div className="mb-6 flex items-center gap-2"><Target className="h-5 w-5 text-purple-600" /><div><h3 className={headerClasses}>Category Imprint</h3><p className={subHeaderClasses}>Your spending fingerprint across top categories.</p></div></div>
            <div className="h-64 sm:h-80 w-full flex items-center justify-center">
              ...
              ) : <div className="flex h-full items-center justify-center text-slate-400 text-sm font-semibold text-center px-6">Requires expenses in at least 3 categories.</div>}
            </div>
          </div>

          <div className={cardClasses}>
            <div className="mb-6 flex items-center gap-2"><Clock className="h-5 w-5 text-orange-600" /><div><h3 className={headerClasses}>Weekday Spending Habits</h3><p className={subHeaderClasses}>When do you spend the most during the week?</p></div></div>
            <div className="h-64 sm:h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
      ...
      {activeTab === 'compare' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Historical Comparison Analysis</h3>
              <p className="text-xs text-slate-500 mt-0.5">Select a month to audit financial performance against previous periods.</p>
            </div>
            <div className="flex items-center gap-2">
              <select
                value={compareYear}
                onChange={e => setCompareYear(Number(e.target.value))}
                className="bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl px-2.5 py-2 outline-none cursor-pointer shadow-sm"
              >
          ...
          <div className={cardClasses}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">MoM DELTAS</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 [&>*:last-child:nth-child(odd)]:col-span-2 sm:[&>*:last-child:nth-child(odd)]:col-span-1">
              {compareMetrics.map((metric: any) => (
                <div key={metric.label} className="rounded-xl border border-slate-100 bg-slate-50/50 p-3 sm:p-4">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{metric.label}</p>
                  <p className="mt-2 text-base sm:text-lg font-black text-slate-900">
                    {metric.isCurrency ? formatCurrency(metric.current) : metric.current}
                  </p>
                  <div className="mt-2 space-y-1 text-xs text-slate-500">
                    <p className="text-[10px] text-slate-400">Prev: {metric.isCurrency ? formatCurrency(metric.previous) : metric.previous}</p>
                    <div className="flex items-center justify-between gap-1 mt-1">
                      <span className="font-bold text-[10px] sm:text-xs text-slate-700">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 mt-0.5">{formatDeltaPct(metric.delta.pct)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className={cardClasses}>
              ...
              <div className="space-y-3">
                {[
                  ...
                  return (
                    <div key={item.label} className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50/50 px-4 py-3 hover:bg-slate-50 transition-colors">
                      <span className="text-xs sm:text-sm font-bold text-slate-700">{item.label}</span>
                      <div className="text-right">
                        <p className="text-sm font-black text-slate-900">{item.isCount ? item.current : formatCurrency(item.current)}</p>
                        <p className={`text-[10px] sm:text-xs font-bold ${
                          delta.direction === 'up' && item.label !== 'Expenses' ? 'text-emerald-600' :
                          delta.direction === 'down' && item.label === 'Expenses' ? 'text-emerald-600' :
                          delta.direction === 'flat' ? 'text-slate-500' : 'text-rose-600'
                        }`}>
      ...
      {activeTab === 'forecasting' && (
        <div className="grid grid-cols-1 gap-6 animate-fadeIn">
          {goalForecastData && !Array.isArray(goalForecastData) ? (
            <div className={cardClasses}>
              <div className="mb-6 flex items-center gap-2"><TrendingUp className="h-5 w-5 text-emerald-600" /><div><h3 className={headerClasses}>Goal Trajectory Forecast</h3><p className={subHeaderClasses}>Projected completion for top priority: <strong className="text-slate-700">{goalForecastData.goalName}</strong></p></div></div>
              ...
              <div className="mt-4 rounded-xl bg-slate-50 p-4 border border-slate-100">
                <p className="text-sm text-slate-600">Based on your last 3 months average savings rate, you are projected to hit this goal in approx. <strong className="font-bold text-emerald-600">{goalForecastData.monthsToCompletion} months</strong>.</p>
              </div>
            </div>
          ) : (
            <div className="flex h-64 items-center justify-center text-slate-400 text-sm font-semibold border border-slate-200 border-dashed rounded-2xl bg-white">
