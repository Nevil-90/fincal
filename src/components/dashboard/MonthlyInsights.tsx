  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-black text-slate-900 dark:text-white">{MONTH_NAMES[month - 1]} {year} — Monthly Report</h3>
      </div>
      ...
      <div className={`bg-gradient-to-r ${healthBg} border rounded-2xl p-4 flex items-center gap-4`}>
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-neutral-400">Financial Health Score</p>
          ...
          <p className="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">Based on savings rate and spending trends</p>
        </div>
      </div>

      {/* Key Stats */}
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-700/70 rounded-2xl p-3.5 text-center shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-neutral-500">Income</p>
          <p className="text-base font-black text-emerald-600 dark:text-emerald-400 mt-1">{formatCurrency(currentIncome)}</p>
        </div>
        <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-700/70 rounded-2xl p-3.5 text-center shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-neutral-500">Expenses</p>
          <p className="text-base font-black text-rose-600 dark:text-rose-400 mt-1">{formatCurrency(currentExpense)}</p>
        </div>
        <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-700/70 rounded-2xl p-3.5 text-center shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-neutral-500">Saved</p>
          <p className={`text-base font-black mt-1 ${savings >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-rose-600 dark:text-rose-400'}`}>
      ...
      {prevExpense > 0 && (
        <div className={`flex items-center gap-3 rounded-2xl p-3.5 border ${
          ...
          <p className={`text-sm font-semibold ${expenseChange < 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'}`}>
            ...
            {expenseChange < 0 && <span className="flex items-center gap-1 mt-1"><PartyPopper className="h-3 w-3 text-emerald-500 dark:text-emerald-400" /> Great job!</span>}
          </p>
        </div>
      )}
            ...
            <div key={cat} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-semibold text-slate-700 dark:text-neutral-300">{cat}</span>
                  <span className="text-sm font-bold text-slate-900 dark:text-white">{formatCurrency(amount)}</span>
                </div>
      ...
      {worstDay && (
        <div className="grid grid-cols-2 gap-3 [&>*:last-child:nth-child(odd)]:col-span-2">
          <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-200 rounded-2xl p-3.5">
            <p className="text-[10px] font-bold uppercase tracking-wider text-rose-500 dark:text-rose-400">Highest Spend Day</p>
            <p className="text-base font-black text-rose-700 dark:text-rose-400 mt-1">{worstDay[0]}</p>
            <p className="text-xs text-rose-500 dark:text-rose-400">{formatCurrency(worstDay[1])} avg</p>
          </div>
          {bestDay && bestDay[0] !== worstDay[0] && (
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 rounded-2xl p-3.5">
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-500 dark:text-emerald-400">Lowest Spend Day</p>
              <p className="text-base font-black text-emerald-700 dark:text-emerald-400 mt-1">{bestDay[0]}</p>
              <p className="text-xs text-emerald-500 dark:text-emerald-400">{formatCurrency(bestDay[1])} avg</p>
            </div>
          )}
        </div>
      )}
