      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-2 pt-5 px-6">
          {STEPS.map((s) => (
            <div key={s.id} className="flex items-center gap-2">
              ...
              {s.id < STEPS.length && (
                <div className={`h-0.5 w-8 rounded-full transition-all ${step > s.id ? 'bg-emerald-400' : 'bg-slate-200'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="p-7">
            ...
            <div className="text-center space-y-4 animate-fade-in">
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-900">
                  <span className="flex items-center gap-2">Welcome, {user.firstName}! <PartyPopper className="h-6 w-6 text-indigo-500" /></span>
                </h2>
          ...
          {step === 2 && (
            <div className="space-y-5 animate-fade-in">
              <div className="text-center">
                <div className="w-14 h-14 bg-gradient-to-br from-emerald-400 to-green-600 rounded-2xl flex items-center justify-center mx-auto shadow-lg shadow-emerald-200">
                  <DollarSign className="h-7 w-7 text-white" />
                </div>
                <h2 className="mt-4 text-xl font-black text-slate-900">What's your opening balance?</h2>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0"
                  className="w-full pl-10 pr-4 py-4 text-2xl font-black text-slate-900 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-center"
                />
              </div>
              <p className="text-xs text-slate-400 text-center">You can skip this and add it later.</p>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-5 animate-fade-in">
              <div className="text-center">
                <div className="w-14 h-14 bg-gradient-to-br from-purple-500 to-violet-600 rounded-2xl flex items-center justify-center mx-auto shadow-lg shadow-purple-200">
                  <Target className="h-7 w-7 text-white" />
                </div>
                <h2 className="mt-4 text-xl font-black text-slate-900">Set a monthly spending limit</h2>
                <p className="mt-1.5 text-sm text-slate-500">
              ...
              <p className="text-xs text-slate-400 text-center">You can change this anytime in Settings.</p>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="text-center space-y-4 animate-fade-in">
              <div className="w-16 h-16 bg-gradient-to-br from-emerald-400 to-green-600 rounded-full flex items-center justify-center mx-auto shadow-lg shadow-emerald-200">
                <Check className="h-9 w-9 text-white" />
              </div>
              <div>
                <h2 className="text-2xl font-black text-slate-900 flex items-center justify-center gap-2">You're all set! <Rocket className="h-6 w-6 text-indigo-500" /></h2>
                <p className="mt-2 text-slate-500 text-sm leading-relaxed">
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-left space-y-2">
                {openingBalance && (
                  <div className="flex items-center gap-2 text-sm text-slate-700">
                    ...
                    <span>Opening balance of <strong>₹{parseFloat(openingBalance).toLocaleString('en-IN')}</strong> will be logged</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-sm text-slate-700">
                  <Check className="h-4 w-4 text-emerald-500 shrink-0" />
